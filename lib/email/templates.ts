...
import { generateEnrollmentEmailTemplate } from "./templateParts/enrollmentEmailTemplate";
import { generateYear3PricingEmailTemplate } from "./templateParts/year3PricingEmailTemplate";
...
export function replaceTemplateVariables(
  ...
  result = result.replace(
    /\{\{parentName\}\}/g,
    escapeHtml(data.parentName || "Parent"),
  );
  result = result.replace(
    /\{\{studentName\}\}/g,
    escapeHtml(data.studentName || "Student"),
  );
  result = result.replace(
    /\{\{courseName\}\}/g,
    escapeHtml(data.courseName || "the course"),
  );
  result = result.replace(
    /\{\{studentEmail\}\}/g,
